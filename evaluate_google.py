...
Uses Google Gemini as the LLM judge.
...
Usage:
    python evaluate.py --groq-key GK --google-key GGK
    python evaluate.py --skip-live                      # log analysis only
    python evaluate.py --groq-key GK --google-key GGK --delay 3.0
"""
...
# Judge model — Gemini 2.5 Flash via Google AI Studio
...
def _make_clients(groq_key: str = "", google_key: str = "") -> dict:
    ...
    if google_key:
        try:
            ...
            clients[PROVIDER_GOOGLE] = _genai.Client(api_key=google_key)
        except ImportError:
            print("⚠️  google-genai package not found. Run: pip install google-genai")
    return clients
...
def judge_response(google_client, scenario: dict, response_text: str,
                   debug: bool = False) -> dict:
    """
    Score a model response using Gemini 2.5 Flash as judge.
    ...
    try:
        ...
        result = google_client.models.generate_content(
            ...
            contents=(
                judge_prompt
                + '\n\nIMPORTANT: Respond with a single valid JSON object only. No markdown, no prose. '
                'Example format: {"accuracy": 4, "helpfulness": 4, "tone": 4, "conciseness": 4, "instruction_following": 4, "reasoning": "Brief reason."}'
            ),
        ...
        if not result.text:
            ...
            raise ValueError(f"Gemini returned empty text. Finish reason: {reason}")
...
def run_evaluation(groq_key: str, google_key: str,
    ...
    print(f"⚖️  Judge: {JUDGE_MODEL} (Google)")
    ...
    for scenario in TEST_SCENARIOS:
        ...
        for model_id, cfg in active_models.items():
            ...
            if not output["success"]:
                ...
                scores         = judge_response(google_client, scenario, output["response"])
                output["scores"] = scores
                time.sleep(0.3)                   # small polite delay for Google
...
if __name__ == "__main__":
    ...
    parser.add_argument("--google-key",  type=str, default=os.getenv("GOOGLE_API_KEY", ""),
                        help="Google AI Studio key (or set GOOGLE_API_KEY env var)")
    ...
    if not args.skip_live:
        ...
        if not args.google_key: missing_keys.append("--google-key (GOOGLE_API_KEY)")

        if missing_keys:
            print("\n❌ Missing required keys for live evaluation:")
            for k in missing_keys:
                print(f"   {k}")
            print("\n   Usage:  python evaluate.py --groq-key GK --google-key GGK")
