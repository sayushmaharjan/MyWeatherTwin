...
MODELS = {
    "llama-3.3-70b-versatile": {
        ...
        "color": "#3b82f6",
        "tier": "primary",
        # Current production model — the one being evaluated
    },
    "openai/gpt-oss-120b": {
        "label": "GPT-OSS 120B (via Groq)",
        "color": "#10b981",
        "tier": "prev-gen",
        # Same family, prior version — directly shows if upgrading to 3.3 was worth it
    },
    "openai/gpt-oss-20b": {
        "label": "GPT-OSS 20B (via Groq)",
        "color": "#f59e0b",
        "tier": "small",
        # Small/cheap model — tests if 70B is even needed for weather insights
    },
    "llama-3.1-8b-instant": {
        "label": "LLaMA 3.1 8B (Baseline-Fast)",
        "color": "#8b5cf6",
        "tier": "fast",
        # Fastest model on Groq — latency/quality tradeoff baseline
    },
}
# NOTE: gemma2-9b-it and mixtral-8x7b-32768 were deprecated and removed from Groq's API.
# The replacements above give you: same-family prev-gen comparison (3.1 70B),
# small model quality floor (LLaMA 3 8B), and speed baseline (3.1 8B instant).
...
def call_model(client: "Groq", model_id: str, scenario: dict, max_tokens: int = 512) -> dict:
    """Call a single model on a scenario. Returns response dict."""
...
def run_evaluation(groq_key: str, output_path: str = "eval_results.json",
                   rate_limit_delay: float = 2.0) -> dict:
    """
    Run full evaluation across all models and scenarios.
    Includes rate limit delay between calls to avoid hitting 429s.
    """
    if not GROQ_AVAILABLE:
        print("❌ Cannot run live evaluation without groq package.")
        return {}
    ...
    results = {
        ...
        "model_results": {model_id: [] for model_id in MODELS},
        "scenario_results": {},
        "model_summary": {},
    }

    total_calls = len(MODELS) * len(TEST_SCENARIOS)
    call_num = 0

    print(f"\n{'='*60}")
    print(f"WeatherTwin LLM Evaluation")
    print(f"Models: {len(MODELS)} | Scenarios: {len(TEST_SCENARIOS)} | Total calls: {total_calls}")
    print(f"{'='*60}\n")
    ...
    for scenario in TEST_SCENARIOS:
        ...
        for model_id in MODELS:
            call_num += 1
            print(f"   [{call_num}/{total_calls}] {MODELS[model_id]['label']}... ", end="", flush=True)

            # Call the model
            output = call_model(client, model_id, scenario)
            time.sleep(rate_limit_delay)  # Respect rate limits

            if not output["success"]:
                print(f"❌ ERROR: {output['error'][:60]}")
            else:
                print(f"✅ {output['latency_ms']:.0f}ms | {output['total_tokens']} tokens", end="")

                # Judge the response (only for successful calls)
                print(" | judging...", end="", flush=True)
                scores = judge_response(client, scenario, output["response"], debug=False)
...
if __name__ == "__main__":
    ...
    if not args.skip_live:
        if not args.groq_key:
            print("\n❌ No Groq API key provided. Use --groq-key or set GROQ_API_KEY.")
            print("   Run with --skip-live to only analyze logs.\n")
        else:
            ...
            # Merge log analysis into eval results
        ...
        print("   To run full eval: python evaluate.py --groq-key YOUR_KEY")